...
@Component({
  selector: 'app-day2-stage2',
  templateUrl: './day2-stage2.component.html',
  styleUrls: ['./day2-stage2.component.css']
})
export class Day2Stage2Component implements OnInit {

  public input =
    '1,0,0,3,1,1,2,3,1,3,4,3,1,5,0,3,2,10,1,19,1,19,9,23,1,23,6,27,2,27,13,31,1,10,31,35,1,10,35,' +
  ...
  findNumber(toFind: number) {
    let run = true;
    for (let noun = 0; noun < 100 && run; noun++) {
      for (let verb = 0; verb < 100 && run; verb++) {
        this.inputArray = this.input.split(',').map(value => {
          return parseInt(value, 10);
        });
        this.inputArray[1] = noun;
        this.inputArray[2] = verb;
        const result = this.runCode(this.inputArray)[0];
        if (result === toFind) {
          run = false;
          this.output = (100 * noun + verb).toString();
        }
      }
    }
  }
