...
@Component({
  selector: 'app-day3-stage1',
  templateUrl: './day3-stage1.component.html',
  styleUrls: ['./day3-stage1.component.css']
})
export class Day3Stage1Component implements OnInit {
  public testInput1 =
  ...
  public testCommonPoints: string[] = [];
  ...
  runTest() {
    ...
    this.testCommonPoints = Wire1Array.filter(value => {
      return Wire2Array.includes(value);
    });

    console.log(this.testCommonPoints);

    const results = [];
    this.testCommonPoints.forEach(element => {
      const parts = element.split('/').map(value => {
        return parseInt(value, 10);
      });
      results.push(Math.abs(parts[0]) + Math.abs(parts[1]));
    });
  ...
  doInstruction(instruction: string, wire: Wire) {
    switch (instruction.substr(0, 1)) {
      case 'U':
        this.goUp(instruction.substr(1), wire);
        break;
      case 'D':
        this.goDown(instruction.substr(1), wire);
        break;
      case 'L':
        this.goLeft(instruction.substr(1), wire);
        break;
      case 'R':
        this.goRight(instruction.substr(1), wire);
        break;
    }
  }
  goUp(lenghtStr: string, wire: Wire) {
    const length = parseInt(lenghtStr, 10);
    for (let i = 0; i < length; i++) {
      wire.x++;
      wire.pathArray.add(wire.x + '/' + wire.y);
    }
  }
  goDown(lenghtStr: string, wire: Wire) {
    const length = parseInt(lenghtStr, 10);
    for (let i = 0; i < length; i++) {
      wire.x--;
      wire.pathArray.add(wire.x + '/' + wire.y);
  ...
  goLeft(lenghtStr: string, wire: Wire) {
    const length = parseInt(lenghtStr, 10);
    for (let i = 0; i < length; i++) {
      wire.y--;
      wire.pathArray.add(wire.x + '/' + wire.y);
  ...
  goRight(lenghtStr: string, wire: Wire) {
    const length = parseInt(lenghtStr, 10);
    for (let i = 0; i < length; i++) {
      wire.y++;
      wire.pathArray.add(wire.x + '/' + wire.y);
...
class Wire {
  public pathArray: Set<string> = new Set<string>();
  public x = 0;
  public y = 0;
}
